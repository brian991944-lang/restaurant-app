...
export default function AddIngredientModal({ isOpen, onClose, onSave, initialData }: AddIngredientModalProps) {
    ...
    return (
        <div style={{
            ...
            <div className="glass-panel" style={{ width: '95%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto', overflowX: 'hidden', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'fadeIn 0.2s ease-out', background: 'var(--bg-primary)', margin: 'auto' }}>
                    ...
                    {currentType === 'PROCESSED' && (
                        <div style={{ display: 'grid', gridTemplateColumns: isPortioned ? '1fr 1fr 1fr' : '1fr', gap: '1rem', background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', justifyContent: 'center' }}>
                                <label style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>Live Calc: Cost Preview</label>
                                {conversionError ? (
