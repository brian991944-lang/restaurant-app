...
export default function MetricPriceSection({
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border)', background: 'rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', flex: 1 }}>
                ...
                {hasRecipeConstraints ? (
                    ...
                    <SearchableSelect
                        ...
                        options={ALLOWED_METRICS.filter(m => (currentType === 'PROCESSED' && isPortioned) ? m.toLowerCase() === 'units' : true).map(m => ({ value: m, label: locale === 'es' && m === 'Units' ? 'Unidades' : m }))}
