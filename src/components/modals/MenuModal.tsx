                                                ...
                                                <div style={{ display: 'flex', flex: 1, paddingRight: '1rem' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <SearchableSelect
                                                            ...
                                                            onChange={(val) => {
                                                                const selectedOption = cloverModifiersOpt.find(m => m.id === val);
                                                                setModifiers(modifiers.map(m => m.id === mod.id ? {
                                                                    ...m,
                                                                    cloverModifierId: val,
                                                                    name: selectedOption ? selectedOption.name : ''
                                                                } : m));
                                                            }}
                                                            options={[...cloverModifiersOpt].sort((a, b) => (a.name || '').localeCompare(b.name || '')).map(m => ({ value: m.id, label: `${m.name} (${m.group})` }))}
                                                            placeholder="Select Clover Modifier..."
                                                        />
                                                    </div>
                                                </div>
