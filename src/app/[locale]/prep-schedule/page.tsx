...
import { getTeamMembers, addTeamMember, removeTeamMember, getPrepTaskItems, addPrepTaskItem, removePrepTaskItem } from '@/app/actions/teamTasks';
...
export default function PrepSchedulePage() {
    ...
    const loadDataForTab = async (tab: 'morning' | 'night' | 'recurring' | 'completed' | 'team') => {
        ...
        if (tab === 'morning') {
            ...
            const [members, tasks, cats] = await Promise.all([getTeamMembers(), getPrepTaskItems(), getCategories()]);
    ...
    const handleAddPrepTask = async () => {
        const name = prompt("Enter Prep Task name:");
        if (!name) return;
        const catId = categories[0]?.id;
        if (!catId) return alert("Must have at least one category to assign.");
        setIsLoading(true);
        await addPrepTaskItem(name, catId, 'units');
        ...
        <div style={{ display: 'flex', gap: '2rem' }}>
            ...
            <div className="glass-panel" style={{ flex: 1, padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1rem' }}>
                    ...
                    <button onClick={handleAddPrepTask} className="btn-primary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.9rem', background: 'linear-gradient(135deg, #a855f7, #6b21a8)' }}>+ Add Task</button>
                </div>
                {prepItems.map(t => (
                    <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '0.8rem 1rem', borderRadius: '8px' }}>
                        <span>{t.name}</span>
