...
async function run() {
    try {
        console.log("Starting Produce & Vegetales y Frutas Merge & Deduplication...");
        ...
        if (targetCategory && oldCategory) {
            console.log("Updating nameEs of target category...");
            await prisma.category.update({
                where: { id: targetCategory.id },
                data: { nameEs: 'Vegetales y Frutas' }
            });

            console.log("Moving ingredients from Vegetales y Frutas to Produce...");
            await prisma.ingredient.updateMany({
                where: { categoryId: oldCategory.id },
                data: { categoryId: targetCategory.id }
            });

            console.log("Deleting old Vegetales y Frutas category...");
            await prisma.category.delete({ where: { id: oldCategory.id } });
        } else if (oldCategory && !targetCategory) {
            console.log("Renaming Vegetales y Frutas to Produce...");
            targetCategory = await prisma.category.update({
                where: { id: oldCategory.id },
                data: { name: 'Produce', nameEs: 'Vegetales y Frutas' }
            });
            oldCategory = null;
        } else if (targetCategory && !oldCategory) {
            console.log("Vegetales y Frutas not found, just updating Produce nameEs...");
            await prisma.category.update({
                where: { id: targetCategory.id },
                data: { nameEs: 'Vegetales y Frutas' }
            });
            ...
            targetCategory = await prisma.category.create({
                data: { name: 'Produce', nameEs: 'Vegetales y Frutas', department: 'FOOD' }
            });
        }
