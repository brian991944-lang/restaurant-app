...
const filePath = path.join(__dirname, '../src/app/[locale]/recetario/page.tsx');
let content = fs.readFileSync(filePath, 'utf8');

// 1. Initial State in handleCreateNew
content = content.replace(
    /procedureJson: JSON.stringify\(\[''\]\),/g,
    `procedureJson: JSON.stringify(['']),
            platingTracksJson: JSON.stringify({ tracks: [], rows: [] }),
            mediaJson: JSON.stringify({ finalPhotoUrl: '' }),`
);

// 2. Parse State at top of render Editor/Viewer
let parseIndex = content.indexOf(`let procList: string[] = [];`);
let chunk1 = `    let procList: string[] = [];
...
`;
content = content.replace(/let procList: string\[\] = \[\];\s*try \{ procList = JSON.parse\(docData.procedureJson || '\[\]'\); \} catch \{ \}/, chunk1);

// 3. Overview section + Final Plated Photo
let overviewChunk = `{/* Overview */}
...
content = content.replace(/{\/\* Overview \*\/}\s*<div style={{ marginBottom: '2\.5rem' }}>\s*<h3 style={{ fontSize: '1\.1rem', color: 'var\(--text-secondary\)', marginBottom: '0\.5rem', textTransform: 'uppercase', letterSpacing: '1px' }}>{locale === 'es' \? 'Visión General' : 'Overview'}<\/h3>/, overviewChunk);


// 4. Procedure chunk replacement
let procStart = content.indexOf(`{/* Procedure List */}`);
let procEnd = content.indexOf(`{/* Chef Notes */}`);
...
content = content.slice(0, procStart) + platingUI + content.slice(procEnd);

const reactImportIndex = content.indexOf(`import { useState, useEffect } from 'react';`);
content = content.replace(`import { useState, useEffect } from 'react';`, `import React, { useState, useEffect } from 'react';`);

fs.writeFileSync(filePath, content);
console.log('Done modifying recetario/page.tsx');
